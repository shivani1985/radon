...
try{
  ...
  xyz.status(201).send({ msg: savedData });
    ...
    console.error({msg:"there is a error",error:error.message})
...
const postMessage = async function (req, res) {
    ...
    if(!decodedToken) return res.status(400).send({status: false, msg:"token is not valid"})
      ...
      console.log("this is a fault",err.message)
      res.status(500).send({msg:"Error",error:err.message})
    }
  }
    ...
    {
      console.log("this is the mistake",err.message)
      res.status(500).send({msg:"Error",error:err.message})
    }
    ...
    return res.status(200).send({status: true, data: updatedUser})   //200
}
catch(error){
  console.log("this is a mistake",err.message )
  res.status(500).send({msg:"Error".error:err.message})
}
